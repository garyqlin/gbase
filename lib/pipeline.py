...
质量门控管道 — 自动串联 重锤→绘墨→裁决。

流程：
1. 发任务到重锤(8431)，重锤按稳压三步走，输出 JSON 中间件
2. 读重锤的输出 JSON，发到绘墨(8432)做质量评估
3. 读两份 JSON，出最终裁决

升级说明 (2026-05-15):
- 中间文件路径统一: /tmp/ → data/pipelines/{pid}/
- 管道裁决升级为 LLM 裁决（通过绘墨执行）
...
async def _call_arm(url: str, message: str, max_seconds: int = 120) -> dict:
    """HTTP POST 到战甲的 /ask 或 /ink/evaluate 端口，返回解析后的 JSON。"""
    import aiohttp
    ...
    try:
        ...
        return {"status": "error", "error": f"战甲响应超时（{max_seconds}s）"}
...
async def run_gate(
    ...
    # ── 步骤 1：重锤做代码审查 ──
    logger.info("[管道 %s] 步骤1: 重锤代码审查 - %s", pid, target_project)
    ...
    logger.info("[管道 %s] 重锤结果已保存: %s", pid, hammer_file)

    if hammer_result["status"] != "ok":
        result = {
            "pipeline_id": pid,
            "status": "failed",
            "error": f"重锤执行失败: {hammer_result.get('error', '未知错误')}",
            "steps": steps,
            "hammer": None,
            "ink": None,
            "verdict": {"passed": False, "report": "门控中断：重锤执行失败"},
        }
        _write_result(pid, result)
        _record_gate_to_mirror(pid, "failed", task_description, target_project, "重锤执行失败")
        return result

    # 读取重锤的中间 JSON 状态文件（新路径）
    hammer_summary = await _read_json_midfile(h2, "重锤的 step2 输出（未找到）")

    # ── 步骤 2：绘墨做质量评估 ──
    logger.info("[管道 %s] 步骤2: 绘墨质量评估 - %s", pid, target_project)
    ...
    logger.info("[管道 %s] 绘墨结果已保存: %s", pid, ink_file)

    if ink_result["status"] != "ok":
        result = {
            "pipeline_id": pid,
            "status": "failed",
            "error": f"绘墨执行失败: {ink_result.get('error', '未知错误')}",
            "steps": steps,
            "hammer": {"file": str(hammer_file), "summary": hammer_summary},
            "ink": None,
            "verdict": {"passed": False, "report": "门控中断：绘墨执行失败"},
        }
        _write_result(pid, result)
        _record_gate_to_mirror(pid, "failed", task_description, target_project, "绘墨执行失败")
        return result

    ink_summary = await _read_json_midfile(i2, "绘墨的 step2 输出（未找到）")

    # ── 步骤 3：裁决 ──
    logger.info("[管道 %s] 步骤3: 裁决", pid)

    if llm_verdict and ink_result["status"] == "ok" and len(ink_result.get("reply", "")) > 100:
        # LLM 裁决：用绘墨的完整回复做语义分析
