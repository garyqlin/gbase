...
from .schema import (
    CognitionSlice, CognitionType, FeedbackType, 
    now_iso
)
...
class CognitionAction:
    """认知决策结果"""
    def __init__(self, action: str = "pass", priority: str = "LOW",
                 message: str = "", matches: list = None):
...
class CognitionEngine:
    ...
    def process(self, user_message: str, agent_name: str = "",
                session_history: list = None) -> CognitionAction:
        """
        scan → decide → 返回决策
    ...
    def decide(self, matches: list) -> CognitionAction:
        ...
        # 判断是否有方向纠偏
        has_correction = any(
            m.cognition_type == CognitionType.DIRECTION_CORRECTION
            and m.confidence >= 0.5 for m in matches
        )
        ...
        if has_correction:
            ...
            top_correction = next(
                m for m in matches 
                if m.cognition_type == CognitionType.DIRECTION_CORRECTION
            )
            messages = []
            for m in matches[:3]:
                ctype_label = m.cognition_type.value
                lesson = (m.strategy_layer.lesson[:80] 
                         if m.strategy_layer else "无内容")
            ...
            # 高置信度 → 提醒
            lesson = (top.strategy_layer.lesson[:100] 
                     if top.strategy_layer else "无内容")
            return CognitionAction(
                action="warn",
                priority="HIGH",
                message=f"历史经验提醒：{lesson}",
                matches=matches
            )
    ...
    def after_action(self, action: CognitionAction,
        ...
        for m in action.matches:
            ...
            try:
                self.store.record_feedback(
                    m.id, ftype, agent_name, user_feedback_raw
                )
            except Exception as e:
                __import__('logging').getLogger(__name__).error(
                    f"Cognition feedback failed: {e}")

    def _infer_feedback_type(self, text: str) -> Optional[FeedbackType]:
        """从羽非的反馈原话推断反馈类型"""
        text_lower = text.lower()
        
        if any(kw in text_lower for kw in ['好的', '可以', '没错', 
                                             '是的', '就这样', '挺好', '采纳']):
            return FeedbackType.ADOPTED
        elif text_lower.startswith('对') or text_lower == '对':
            return FeedbackType.ADOPTED
        elif any(kw in text_lower for kw in ['不用', '不需要', '不必要', 
                                               '不必', '过']):
            return FeedbackType.IGNORED
        elif any(kw in text_lower for kw in ['不对', '错了', '不是这样', 
                                               '理解错', '不是这个']):
    ...
    def seed_default(self, logs: list = None):
        ...
        if logs:
            ...
            distiller = NeocortexDistiller(
                apprentice_dir="/opt/orange-arm-v2/apprentice/logs/",
