...
SIGNAL_PATTERNS = {
    ...
    CognitionType.TASK_PATTERN: re.compile(
        r'(?i)(帮我|查一下|看看|研究|分析|怎么|如何|什么方式|步骤|流程)'
    ),
}
...
class Scanner:
    ...
    def scan_concept(self, message: str, agent: str = "") -> List[CognitionSlice]:
        ...
        for kw in scene_keywords:
            results.extend(self.store.search_by_concept(
                scene=kw, agent=agent
            ))
    ...
    def scan_strategy(self, message: str, agent: str = "") -> List[CognitionSlice]:
        """
        策略层：从认知库中按内容片段匹配策略。
        """
    ...
    def full_scan(self, message: str, agent: str = "",
        ...
        if signal_types:
            for st in signal_types:
                ...
                for s in self.store.search_by_signal(
                    keywords=[kw], agent=agent
                ):
